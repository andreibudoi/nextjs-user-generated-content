      ...
      <DialogContent className='flex flex-col max-w-3xl overflow-hidden'>
        ...
        <form
          onSubmit={(e) => {
            e.preventDefault();
            try {
              createPost.mutate({ name, content: JSON.stringify(editor?.getJSON()) });
            } catch (_) {
            }
          }}
        ...
        >
          ...
          <div className='space-y-2'>
            <EditorContent
              className='flex flex-grow min-h-[10rem] max-h-[20rem] w-full rounded-md border border-input overflow-hidden'
              editor={editor}/>
